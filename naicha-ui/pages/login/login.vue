<template>
	<view class="container">
		...
		<view class="bottom">
			...
			<button type="primary" size="default" class="login-btn" v-if="appProperties.testUserLoginEnabled" @tap="showPop = true" :disabled="logining">测试号登录</button>
...
export default {
	...
	methods: {
		...
		loginForTest() {
			...
			console.log('测试号登录');
			that.$request('/api-app/user/login/dev?secretPassword=' + this.secretPassword, 'post').then(result => {
				that.$store.commit('login', result.data);
				uni.reLaunch({
					url: '/pages/mine/mine'
				});
