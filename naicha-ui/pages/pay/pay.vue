	...
	<view class="container position-relative">
					...
					<list-cell>
						<view class="w-100 d-flex flex-column">
							<view class="d-flex align-items-center font-size-base text-color-base">
								...
								<view class="mr-10">{{ new Date(new Date().getTime() + 2400 * 1000).toLocaleTimeString() }}</view>
							</view>
						</view>
					</list-cell>
				</template>
		...
		<modal :show="ensureAddressModalVisible" custom :mask-closable="false" width="90%">
			<view class="modal-content">
				...
				<button type="primary" class="pay_btn" @tap="createOrder">确认并付款</button>
			</view>
		</modal>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		createOrder() {
			...
			that.$request('/api-app/order', 'post', params).then(result => {
				that.$store.commit('set_cart', []);
				that.$msg('下单成功');
				that.pay(result.data);
			});
		},
		...
		pay(orderNo) {
			...
			let path = '/api-app/order/pay/weixin?orderNo=' + orderNo + '&ip=null';
