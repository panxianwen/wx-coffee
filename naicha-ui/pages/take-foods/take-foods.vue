				...
				<view class="order-box">
					<view class="bg-white">
										...
										<view class="steps d-flex flex-column">
											<view class="steps__img-column">
												...
												<view class="steps__img-column-item" v-if="order.takeType == '外卖配送'">
													<image src="/static/images/order/delivery_selected.png" v-if="order.orderStatus == '配送中' || order.orderStatus == '已送达'"></image>
											...
											<view class="steps__text-column " >
												...
												<view class="steps__img-column-item" v-if="order.takeType == '外卖配送'">
													<view class="steps__column-item-line"></view>
													<view class="steps__text-column-item-text" style="color: lightgrey;">{{order.orderStatus == '已送达' ? '已送达' : '配送中'}}</view>
													<view class="steps__column-item-line bg-transparent" ></view>
												</view>
						...
						<list-cell :hover="false"  >
							<view class="">
								<button type="primary" size="mini" style="margin-right: 5px;" @tap="calcelOrder(order.orderNo)" :disabled="order.orderStatus != '未付款'" >取消订单</button>
								<button type="primary" size="mini"  class=" right" :disabled="paying" v-if="order.orderStatus == '未付款'" @tap="pay(order.orderNo)">去付款</button>
								<button type="primary" size="mini"  class=" right" v-if="order.orderStatus == '请取餐' || order.orderStatus == '已送达'" 
									@tap="comfirmRedeive(order.orderNo)">确认收货</button>
							</view>
						</list-cell>
						<!-- 额外操作 end -->
...
export default {
	...
	methods: {
		getHandlingOrders() {
			let that = this;
			that.$request('/api-app/order/notCompleted', 'get').then(result => {
		...
		calcelOrder(orderNo){
			...
			uni.showModal({
				...
				success: function(e){
					if(e.confirm){
						that.$request("/api-app/order/cancel/" + orderNo, "delete").then(result => {
		...
		comfirmRedeive(orderNo){
			let that = this;
			that.$request("/api-app/order/confirmReceive/" + orderNo, "put").then(result => {
				that.getHandlingOrders();
				that.$msg("已确认收货")
			})
		},
		...
		pay(orderNo) {
			...
			let path = '/api-app/order/pay/weixin?orderNo=' + orderNo ;
			that.$request(path, 'post', null, errorMsg => {
				// 后台发起的微信预支付失败
				uni.reLaunch({
