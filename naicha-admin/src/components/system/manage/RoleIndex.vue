      ...
      <el-form>
        <el-form-item>
          ...
          <el-tree
                  ...
                  node-key="permission"
                  :props="defaultProps"
                  :default-expanded-keys="['system:address', 'system:student', 'system:worker', 'system:taskLog']"
  ...
  export default {
    ...
    methods: {
      ...
      setPermissions() {
        ...
        // let keys = this.$refs.permissionTree.getCheckedKeys(true)  // 获取被选中的key (是否只是叶子节点)
        let permissions = [];
        nodes.forEach(node => {
          if (node.permission) {
