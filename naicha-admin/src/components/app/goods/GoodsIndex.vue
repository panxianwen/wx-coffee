  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        //   ? 'http://yuqy.club:9002/api-system/goodsAdmin/image' : 'http://localhost:9002/api-system/goodsAdmin/image',
        // // 商品图片的基础路径
        // goodsImageBaseUrl: process.env.NODE_ENV === 'production'
        //   ? 'http://yuqy.club:9002/static/image/' : 'http://localhost:9002/static/image/',
        // // 图片上传的url
        fileUploadUrl: `${process.env.VUE_APP_BASE_API}/api-system/goodsAdmin/image`,
        // 商品图片的基础路径
        goodsImageBaseUrl: `${process.env.VUE_APP_BASE_API}/static/image`,
      ...
      handelConfirm() {
        this.$refs['form'].validate(valid => {
          ...
          if (this.formTitle.startsWith('添加')) {
            addGoodsAdmin(this.formData).then(() => {
              this.$notify.success("添加成功");
              this.getGoodsAdmins()
            })
          } else if (this.formTitle.startsWith('更新')) {
            updateGoodsAdmin(this.formData).then(() => {
      ...
      handelGoodsPropertyFormConfirm() {
        ...
        this.$refs['goodsPropertyForm'].validate(valid => {
          ...
          if (that.goodsPropertyFormTitle.startsWith('添加')) {
            ...
            addGoodsProperty(that.goodsPropertyFormData).then(() => {
              that.$notify.success("添加成功");
              that.refreshGoods()
            })
          } else if (this.goodsPropertyFormTitle.startsWith('更新')) {
            updateGoodsProperty(this.goodsPropertyFormData).then(() => {
              this.$notify.success("更新成功");
              that.refreshGoods()
            })
          }
